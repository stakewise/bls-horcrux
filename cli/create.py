...
def handle_rsa_keys(
    total: int, my_rsa_public_key_file: str, all_rsa_public_keys_file: str
) -> Tuple[RsaKey, RsaKey, List[str]]:
    """
    ...
    print(
        f"Saved RSA public key to {my_rsa_public_key_file}. Please share it with other horcruxes."
    )

    print(
        f"Waiting for all the horcruxes RSA public keys at {all_rsa_public_keys_file}."
    )
...
def handle_dispatcher(
    ...
    if not offline_mode:
        ...
        print(
            f"Saved dispatcher input to {input_file}. Please submit it to the dispatcher server."
        )
...
@click.password_option(
    ...
    help=(
        "The password that will secure your keystore. You will need to re-enter this to decrypt them when "
...
def create(
    ...
    if threshold > total:
        raise click.BadParameter(
            "Threshold cannot be larger the number of total horcrux holders."
        )
    ...
    print(
        f"Generating BLS keypair with Shamir's secret sharing: total shares={total}, threshold={threshold}"
    )
    (
        my_bls_public_key,
